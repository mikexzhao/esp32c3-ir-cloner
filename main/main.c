...
static const char ota_html[] =
    ...
    "color: #f8fafc; font-family: -apple-system, BlinkMacSystemFont, \"Segoe "
    "UI\", Roboto, sans-serif; display: flex; justify-content: center; "
    "align-items: center; min-height: 100vh; margin: 0; }"
    ".card { background: rgba(30, 41, 59, 0.7); backdrop-filter: blur(10px); "
    "border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 16px; padding: "
    "32px; width: 90%; max-width: 400px; box-shadow: 0 10px 25px -5px "
    "rgba(0,0,0,0.3); }"
    "h2 { margin-top: 0; font-size: 24px; font-weight: 600; background: "
    "linear-gradient(to right, #38bdf8, #818cf8); -webkit-background-clip: "
    "text; -webkit-text-fill-color: transparent; text-align: center; }"
    ".input-group { margin-bottom: 20px; }"
    "label { display: block; font-size: 14px; font-weight: 500; margin-bottom: "
    "6px; color: #94a3b8; }"
    "input[type=\"text\"], input[type=\"password\"] { width: 100%; padding: "
    "12px; border: 1px solid rgba(255, 255, 255, 0.1); border-radius: 8px; "
    "background: rgba(15, 23, 42, 0.6); color: #fff; box-sizing: border-box; "
    "font-size: 14px; transition: border-color 0.2s; }"
    "input:focus { outline: none; border-color: #6366f1; }"
    "button { width: 100%; padding: 12px; background: linear-gradient(to "
    "right, #4f46e5, #6366f1); border: none; border-radius: 8px; color: #fff; "
    "font-size: 16px; font-weight: 600; cursor: pointer; box-shadow: 0 4px 6px "
    "-1px rgba(79, 70, 229, 0.2); transition: transform 0.1s, box-shadow 0.1s; "
    "}"
    "button:active { transform: scale(0.98); }"
    ".footer { text-align: center; margin-top: 24px; font-size: 12px; color: "
    "#64748b; }"
    "</style>"
    "</head>"
    "<body>"
    ...
    "placeholder=\"Enter local Wi-Fi SSID\" required>"
    "        </div>"
    "        <div class=\"input-group\">"
    ...
    "placeholder=\"http://192.168.x.x/firmware.bin\" "
    "value=\"" OTA_DEFAULT_URL "\" required>"
    "        </div>"
    "        <button type=\"submit\">Start OTA Update</button>"
    "    </form>"
    "    <div class=\"footer\">EtonTech IR Cloner v" FW_VERSION "</div>"
    "</div>"
    "</body>"
    "</html>";

// Helper to set VISLED state
static void set_visled(int level) { gpio_set_level(VISLED, level); }

// Helper to set IRLED state
static void set_irled(int level) { gpio_set_level(IRLED, level); }
...
static void play_carrier(uint32_t duration_us) {
  ...
  while ((esp_timer_get_time() - start) < duration_us) {
    set_irled(IRLED_ON);
    esp_rom_delay_us(
        13); // 38kHz carrier -> 26.3us period (~13us HIGH / ~13us LOW)
    set_irled(IRLED_OFF);
    esp_rom_delay_us(13);
  }
}
...
static void init_gpios(void) {
  ...
  ESP_LOGI(TAG, "Region Mode: %s (GPIO10 Level: %d)", region ? "NA" : "EU",
           region);
}
...
static esp_err_t get_form_value(const char *buf, const char *key, char *val,
                                size_t val_len) {
...
static esp_err_t post_handler(httpd_req_t *req) {
  ...
  if (get_form_value(buf, "ssid", s_ota_ssid, sizeof(s_ota_ssid)) == ESP_OK &&
    ...
    if (get_form_value(buf, "password", s_ota_pass, sizeof(s_ota_pass)) !=
        ESP_OK) {
      s_ota_pass[0] = '\0';
    }

    ESP_LOGI(TAG, "Parsed form payload: SSID=%s, URL=%s", s_ota_ssid,
             s_ota_url);

    const char *resp =
        "<html><head><meta name='viewport' content='width=device-width, "
        "initial-scale=1'><style>body{background:#0f172a;color:#fff;font-"
        "family:sans-serif;display:flex;justify-content:center;align-items:"
        "center;height:100vh;margin:0;}div{text-align:center;padding:24px;"
        "border:1px solid "
        "rgba(255,255,255,0.1);border-radius:12px;background:rgba(30,41,59,0.8)"
        ";}</style></head><body><div><h2>Update Initialized</h2><p>Connecting "
        "to Wi-Fi to download firmware...</p><p>Check serial monitor for "
...
static httpd_handle_t start_webserver(void) {
  ...
  if (httpd_start(&server, &config) == ESP_OK) {
    ...
    httpd_uri_t post_uri = {.uri = "/callback",
                            .method = HTTP_POST,
                            .handler = post_handler,
                            .user_ctx = NULL};
    httpd_register_uri_handler(server, &post_uri);

    httpd_uri_t default_get_uri = {.uri = "*",
                                   .method = HTTP_GET,
...
static void wifi_init_softap(void) {
  ...
  // Low Wi-Fi TX Power (approx. 2dBm output) to save battery power during
  ...
  while (1) {
    ...
    if (button_pressed) {
      if (!button_was_pressed) {
        ...
        if (s_state == STATE_READY_TO_PLAY &&
            (now_ms - button_press_start_time) >= LONG_PRESS_TIME_MS) {
          ESP_LOGI(TAG, "Long press detected! Clearing RAM and restarting to "
                        "enter OTA mode...");
          for (int i = 0; i < 6; i++) {
            set_visled(i % 2);
          ...
          esp_restart(); // Reboot system so it goes back to OTA configuration
                         // window
        }
      }
    } else {
      if (button_was_pressed) {
        ...
        if (duration < LONG_PRESS_TIME_MS) {
          if (s_state == STATE_READY_TO_PLAY) {
            ...
            ESP_LOGW(TAG, "No IR code recorded yet. Point remote at receiver "
                          "and press button.");
    ...
    case STATE_RECORDING: {
      ...
      while (1) {
        ...
        set_visled(level); // HIGH (no carrier) -> VISLED ON, LOW (carrier
                           // detected) -> VISLED OFF
        ...
        if ((now - start_time) > MAX_RECORD_TIME_US) {
          ESP_LOGI(TAG,
                   "Maximum recording duration reached. Recording finished.");
          break;
        }
      ...
      if (s_transition_count > 0) {
        ESP_LOGI(TAG, "Successfully recorded %d transitions.",
                 s_transition_count);
...
static void ota_task(void *pvParameters) {
  ...
  strncpy((char *)sta_config.sta.password, s_ota_pass,
  ...
  esp_http_client_config_t http_config = {
      ...
      .crt_bundle_attach =
          esp_crt_bundle_attach, // Attach Root CAs for standard HTTPS
  };
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG, "OTA Update failed with error code: %s. Rebooting...",
             esp_err_to_name(err));
    vTaskDelay(pdMS_TO_TICKS(2000));
    esp_restart();
  }
...
void app_main(void) {
  ...
  ESP_LOGI(TAG, "Firmware Version: " FW_VERSION);
  ESP_LOGI(TAG, "Release Date: " FW_DATE);
  ESP_LOGI(TAG, "Author: " FW_AUTHOR);
  ...
  const esp_timer_create_args_t timer_args = {.callback = &ota_timeout_callback,
                                              .name = "ota_timeout"};
  ...
  ESP_LOGI(TAG, "Inactivity timer set to 5 minutes. Captive portal "
